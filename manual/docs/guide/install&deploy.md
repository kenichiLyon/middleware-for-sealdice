...
由于个人能力有限，暂时未对两方案进行日志与错误提示的适配，已有计划适配日志与错误提示，目前建议用户部署完成 (连接成功) 后，在对接了 `sealdice-core` 的账号内尝试测试图片发送、语音发送等功能是否正常使用，如出现异常，请先按照 [故障排除](https://sealdice.js.org/guide/troubleshooting.html) 进行排查，如无法解决，请在本项目仓库的 issue 区及时反馈。
